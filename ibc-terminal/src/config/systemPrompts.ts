...
// Generate a detailed analysis of the player's problem-solving behavior
export const generateSessionAnalysis = (
  ...
  const puzzleInteractions = worldData.puzzles.map((puzzle) => {
    ...
    const averageHesitation = hesitationMoments.length > 0 
      ? hesitationMoments.reduce((sum: number, msg: any) => {
          return sum + (msg.metrics?.hesitations?.reduce((s: number, h: any) => s + h.duration, 0) || 0);
    ...
    return {
      ...
      timeToSolution: solutionMessage && relevantMessages[0] 
...
${
  pi.hesitationCount > 0
    ? `- Average Hesitation Duration: ${Math.round(pi.averageHesitationDuration)}ms`
    : ""
}
...
- Impact of environmental cues on problem-solving time: ${assessEnvironmentalCueImpact(puzzleInteractions)}`
    : ""
}
...
- Strength of functional fixedness effect: ${assessFixednessEffectStrength(puzzleInteractions, variant)}
- Quality of puzzle design: ${assessPuzzleDesignQuality(puzzleInteractions, worldData.puzzles)}
- Suggestions for experimental improvement: ${suggestExperimentalImprovements(puzzleInteractions, worldData, variant)}
...
function getFixationLevel(interactions: any[], puzzles: any[]) {
  ...
  puzzles.forEach(puzzle => {
    const puzzleMessages = interactions.filter(
      msg => 
        msg.role === "user" && 
        msg.content?.toLowerCase().includes(puzzle.fixedFunctionObject.toLowerCase())
    );
    ...
    unconventionalUses += puzzleMessages.filter(
      msg => 
        msg.content?.toLowerCase().includes(puzzle.solution.toLowerCase())
    ).length;
  });
...
function getProblemSolvingApproach(interactions: any[]) {
  ...
  const useCommands = interactions.filter(
    msg => 
      msg.role === "user" && 
      msg.content?.toLowerCase().startsWith("use")
  ).length;
  ...
  if (inspectRatio > 1.5) {
    return {
      ...
      characteristics: "Player carefully examined objects and environment before attempting solutions",
    };
    ...
    return {
      ...
      characteristics: "Player focused on trying solutions with minimal exploration",
    };
  }
}
...
function getAlternativeApproaches(interactions: any[], puzzle: any) {
  ...
  const approaches = [...new Set(
    puzzleInteractions.map(msg => msg.content)
  )];
  
  return approaches.length > 0 
    ? `Player tried ${approaches.length} alternative approaches: ${approaches.slice(0, 3).join("; ")}${approaches.length > 3 ? "..." : ""}`
...
function getBreakthroughMoment(interactions: any[], puzzle: any) {
  ...
  if (solutionIndex <= 0) return "Immediate solution with no clear breakthrough moment";
  ...
  if (prevMessages.length === 0) return "No clear breakthrough trigger identified";
...
function getConceptualTransferEvidence(interactions: any[], puzzles: any[]) {
  ...
  const puzzleSolutions = solvedPuzzles.map(puzzle => {
    const solutionMsg = interactions.find(
      msg => 
        msg.role === "user" && 
        msg.content?.toLowerCase().includes(puzzle.solution.toLowerCase())
  ...
  if (puzzleSolutions.length >= 2) {
    ...
    for (let i = 1; i < puzzleSolutions.length; i++) {
      timeBetweenSolutions.push(puzzleSolutions[i].timestamp - puzzleSolutions[i-1].timestamp);
    }
    
    const decreasingTime = timeBetweenSolutions.length >= 2 && 
      timeBetweenSolutions[timeBetweenSolutions.length - 1] < timeBetweenSolutions[0];
    
    if (decreasingTime) {
      transferEvidence += "Decreasing time between puzzle solutions suggests growing transfer of repurposing concepts. ";
      transferCount++;
    }
  }
...
function assessEnvironmentalCueImpact(puzzleInteractions: any[]) {
  ...
  const averageAttempts = solvedPuzzles.reduce((sum, pi) => sum + pi.attemptCount, 0) / solvedPuzzles.length;
...
function identifyFixednessPatterns(interactions: any[], puzzles: any[]) {
  ...
  puzzles.forEach(puzzle => {
    ...
    if (conventionalUses.length >= 3) {
      patterns.push(`Repeated conventional use attempts with ${puzzle.fixedFunctionObject}`);
    }
  });
  ...
  if (confusionMessages.length > 0) {
    patterns.push("Expressions of confusion or limitation when conventional uses failed");
  }
  ...
  if (hesitationBeforeSolutions.length > 0) {
    patterns.push("Hesitation immediately before arriving at unconventional solutions");
  }
...
function analyzeHesitationPatterns(puzzleInteractions: any[]) {
  ...
  const totalHesitationCount = puzzleInteractions.reduce((sum, pi) => sum + pi.hesitationCount, 0);
...
function comparePuzzleDifficulty(puzzleInteractions: any[]) {
  ...
  const puzzlesByAttempts = [...puzzleInteractions]
    .sort((a, b) => b.attemptCount - a.attemptCount);
  
  // Rank puzzles by solution time (if available)
  const solvedPuzzles = puzzleInteractions.filter(pi => pi.timeToSolution !== null);
  const puzzlesByTime = solvedPuzzles.length > 0 
    ? [...solvedPuzzles].sort((a, b) => (b.timeToSolution || 0) - (a.timeToSolution || 0))
    : [];
  ...
  if (puzzlesByTime.length > 0) {
    difficultyAnalysis += `Most time-consuming puzzle: "${puzzlesByTime[0].puzzle.name}" (${Math.round((puzzlesByTime[0].timeToSolution || 0) / 1000)} seconds). `;
  }
  
  return difficultyAnalysis || "Insufficient data to compare puzzle difficulty";
}

function assessFixednessEffectStrength(puzzleInteractions: any[], variant: string) {
  // Evaluate how strongly functional fixedness affected the participant
  const solvedCount = puzzleInteractions.filter(pi => pi.solutionFound).length;
  const totalPuzzles = puzzleInteractions.length;
  const averageAttempts = puzzleInteractions.reduce((sum, pi) => sum + pi.attemptCount, 0) / totalPuzzles;
  
  // Expected differences between variants
  const variantExpectation = variant === "A" 
  ...
  if (solutionRate < 0.5 || averageAttempts > 5) {
    return `Strong fixedness effect (${solvedCount}/${totalPuzzles} puzzles solved, avg ${averageAttempts.toFixed(1)} attempts). ${variantExpectation}`;
  } else if (solutionRate < 1.0 || averageAttempts > 3) {
    return `Moderate fixedness effect (${solvedCount}/${totalPuzzles} puzzles solved, avg ${averageAttempts.toFixed(1)} attempts). ${variantExpectation}`;
  } else {
    return `Mild fixedness effect (${solvedCount}/${totalPuzzles} puzzles solved, avg ${averageAttempts.toFixed(1)} attempts). ${variantExpectation}`;
  }
}
...
function assessPuzzleDesignQuality(puzzleInteractions: any[], puzzles: any[]) {
  ...
  const instantPuzzles = puzzleInteractions.filter(pi => pi.solutionFound && pi.attemptCount <= 1);
  const goodPuzzles = puzzleInteractions.filter(pi => pi.solutionFound && pi.attemptCount > 1);
  ...
  if (unsolvedPuzzles.length > 0) {
    qualityAssessment += `Potentially too difficult: "${unsolvedPuzzles.map(pi => pi.puzzle.name).join('", "')}". `;
  }
  
  if (instantPuzzles.length > 0) {
    qualityAssessment += `Potentially too obvious: "${instantPuzzles.map(pi => pi.puzzle.name).join('", "')}". `;
  }
  
  if (goodPuzzles.length > 0) {
    qualityAssessment += `Well-calibrated puzzles: "${goodPuzzles.map(pi => pi.puzzle.name).join('", "')}". `;
  }
  
  return qualityAssessment || "Insufficient data to assess puzzle design quality";
}

function suggestExperimentalImprovements(puzzleInteractions: any[], worldData: any, variant: string) {
  // Suggest improvements based on observed patterns
  const suggestions = [];
  
  // Check for puzzles that were too easy
  const easyPuzzles = puzzleInteractions.filter(pi => pi.solutionFound && pi.attemptCount <= 1);
  if (easyPuzzles.length > 0) {
    suggestions.push(`Increase difficulty of "${easyPuzzles.map(pi => pi.puzzle.name).join('", "')}" by strengthening conventional context`);
  }
  ...
  if (hardPuzzles.length > 0) {
    suggestions.push(`Consider adding more subtle cues for "${hardPuzzles.map(pi => pi.puzzle.name).join('", "')}" puzzles`);
  }
  ...
  if (variant === "B") {
    ...
    if (lowInfluencedPuzzles.length > 0) {
      suggestions.push(`Strengthen environmental examples related to "${lowInfluencedPuzzles.map(pi => pi.puzzle.name).join('", "')}" puzzles`);
    }
  }
  
  // General improvements
  const averageHesitation = puzzleInteractions.reduce(
    (sum, pi) => sum + pi.averageHesitationDuration, 0
  ) / Math.max(1, puzzleInteractions.filter(pi => pi.hesitationCount > 0).length);
  
  if (averageHesitation > 1000) {
    suggestions.push("Consider implementing real-time hesitation tracking to dynamically adjust difficulty");
  }
...
function getOverallFixednessLevel(puzzleInteractions: any[]) {
  ...
  const conventionalUses = puzzleInteractions.reduce((sum, pi) => sum + pi.conventionalUseCount, 0);
  const unconventionalUses = puzzleInteractions.reduce((sum, pi) => sum + (pi.attemptCount - pi.conventionalUseCount), 0);
  const fixednessRatio = conventionalUses / Math.max(1, unconventionalUses);

  if (solvedCount / totalPuzzles < 0.5 || averageAttempts > 5 || fixednessRatio > 3) {
    return "High";
  } else if (solvedCount / totalPuzzles < 0.8 || averageAttempts > 3 || fixednessRatio > 1.5) {
    return "Moderate";
  } else {
    return "Low";
  }
...
function getKeyFindings(
  ...
  const solveRate = solvedPuzzles.length / Math.max(1, puzzleInteractions.length);
  
  // Calculate average attempts
  const avgAttempts = puzzleInteractions.reduce((sum, pi) => sum + pi.attemptCount, 0) / 
    Math.max(1, puzzleInteractions.length);
  
  // Calculate average time to solution
  const avgTimeToSolution = solvedPuzzles.length > 0 
    ? solvedPuzzles.reduce((sum, pi) => sum + (pi.timeToSolution || 0), 0) / solvedPuzzles.length
    : 0;
  
  let findings = "";
  
  if (variant === "A") {
    findings += `Control variant showed ${solveRate < 0.5 ? "strong" : solveRate < 0.8 ? "moderate" : "mild"} functional fixedness, with ${solvedPuzzles.length}/${puzzleInteractions.length} puzzles solved and avg ${avgAttempts.toFixed(1)} attempts per puzzle. `;
    
    if (solvedPuzzles.length > 0) {
      findings += `Average solution time of ${Math.round(avgTimeToSolution / 1000)} seconds indicates ${avgTimeToSolution > 180000 ? "significant" : "moderate"} difficulty overcoming functional fixedness. `;
    }
  } else {
    ...
    if (solveRate > expectedControlRate && avgAttempts < expectedControlAttempts) {
      findings += `Environmental priming appears effective, with higher solve rate (${solvedPuzzles.length}/${puzzleInteractions.length}) and lower average attempts (${avgAttempts.toFixed(1)}) than typically seen in control conditions. `;
    } else {
...
function getDunckerAlignmentAssessment(
  ...
  const solvedRate = puzzleInteractions.filter(pi => pi.solutionFound).length / puzzleInteractions.length;
  const avgAttempts = puzzleInteractions.reduce((sum, pi) => sum + pi.attemptCount, 0) / puzzleInteractions.length;
  
  // Expected patterns based on Duncker's research
  const expectedVariantA = "In control condition (w.p.), participants typically solve fewer problems and require more attempts";
  const expectedVariantB = "In experimental condition (a.p.), participants typically struggle more after pre-utilizing objects conventionally";
  
  if (variant === "A") {
    if (solvedRate < 0.7) {
      return `Results strongly align with Duncker's control condition findings - showing functional fixedness with ${Math.round(solvedRate * 100)}% solve rate. ${expectedVariantA}`;
    } else {
      return `Results partially align with Duncker's paradigm, though solve rate (${Math.round(solvedRate * 100)}%) is higher than typical control findings. ${expectedVariantA}`;
    }
  } else {
