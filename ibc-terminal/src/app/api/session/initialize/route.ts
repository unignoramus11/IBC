...
export async function POST(request: NextRequest) {
  try {
    ...
    if (session.history.length > 0) {
      ...
      const modelMessage = session.history.find(msg => msg.role === 'model' || msg.role === 'assistant');
      if (modelMessage?.parts[0]?.text) {
        initialMessage = modelMessage.parts[0].text;
      } else {
        // Fallback if no model message is found
        initialMessage = 'Welcome back to the adventure. Type "help" for available commands.';
      }
    } else {
      ...
      try {
        ...
        initialMessage = stripMarkdown(introMessage || "Welcome to the adventure.");
